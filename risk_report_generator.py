...
NEWS_API_KEY = "enter newsAPI here"

NUM_OF_REPORTS = 5

client = openai.OpenAI(api_key="enter OPENAI_API_KEY HERE")
...
def fetch_articles(query, api_key, total):
    endpoint = f"https://newsapi.org/v2/everything?q={query}&apiKey={api_key}&pageSize=100"
    all_articles = []
    page = 1
    ...
    while total > 0:
        ...
        if total <= 0 or len(data['articles']) < 100:
            break
